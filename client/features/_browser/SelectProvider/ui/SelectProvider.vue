...
onMounted(() => {
  if (!HGetQuery('provider'))
    HSetQuery('provider', 'modrinth')
})
...
<template>
  <UTabs class="basis-1/3" :items="providers" :default-index="providers.findIndex((el) => el.raw === currentProvider)" @change="setProvider($event)" />
</template>
